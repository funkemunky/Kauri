...
public class ReachD extends Check {
    ...
    public void onPacket(Object packet, String packetType, long timeStamp) {
        if (packetType.equals(Packet.Client.ARM_ANIMATION)) {
            ...
            if (getData().getPing() > 400 || (entityData != null && getData().getPing() > 400) || getData().getLastServerPos().hasNotPassed(0) || getData().isLagging()) {
                return;
            }
            ...
            val distance = target.getLocation().distance(origin);

            if(distance > 20) {
                if(vl++ > 2) {
                    flag(distance + ">-20", true, true);
                }
                return;
            }

            List<Vector> vecs = trace.traverse(2, distance, accuracy);
