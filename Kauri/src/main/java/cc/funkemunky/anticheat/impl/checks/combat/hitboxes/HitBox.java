...
public class HitBox extends Check {
    ...
    private BoundingBox getHitbox(LivingEntity entity, CustomLocation l) {
        ...
        return new BoundingBox(0, 0, 0, 0, 0, 0).grow(0.15f,0.15f,0.15f).add((float) l.getX(), (float) l.getY(), (float) l.getZ()).grow((float) dimensions.getX(), (float) dimensions.getY(), (float) dimensions.getZ()).grow(.25f, 0.15f, .25f);
    }
}
