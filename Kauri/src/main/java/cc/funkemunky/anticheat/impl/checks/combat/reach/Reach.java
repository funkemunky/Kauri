...
@Packets(packets = {Packet.Client.ARM_ANIMATION, Packet.Client.USE_ENTITY})
@Init
public class Reach extends Check {
    ...
    public void onPacket(Object packet, String packetType, long timeStamp) {
        ...
        if(packetType.equals(Packet.Client.ARM_ANIMATION)) {
            ...
            if(target != null && getData().getLastLogin().hasPassed(5) && !getData().isServerPos() && getData().getTransPing() < 450) {
                long range = 150 + Math.abs(getData().getTransPing() - getData().getLastTransPing()) * 3;
                val location = getData().getEntityPastLocation().getEstimatedLocation(getData().getTransPing(), range);
                val to = move.getTo().toLocation(getData().getPlayer().getWorld()).add(0, getData().getActionProcessor().isSneaking() ? 1.47f : 1.62f, 0);
                ...
                val traverse = trace.traverse(0, calcDistance, 0.1, 0.02, 2.8, 4);
                float distance = (float)traverse.stream()
                        .filter((vec) -> location.stream().anyMatch((loc) -> getHitbox(target, loc).collides(vec)))
                        .mapToDouble((vec) -> vec.distance(to.toVector()))
                        .min().orElse(0.0D) - (velocity.getLastVelocity().hasNotPassed(5) ? (float) MathUtils.hypot(velocity.getMotionX(), velocity.getMotionZ()) / 1.25f : 0);

                if(distance <= 2) {
                    return;
                }

                if(distance > 3.02F && getData().getMovementProcessor().getLagTicks() == 0) {
                    if(vl++ > 12) {
                ...
                debug((distance > 3.04 ? Color.Green : "") + "distance=" + distance + " vl=" + vl + " range=" + range + " vel=" + getData().getVelocityProcessor().getLastVelocity().getPassed());
            }
        }
    }
    ...
    private BoundingBox getHitbox(LivingEntity entity, CustomLocation l) {
        ...
        return (new BoundingBox(l.toVector(), l.toVector())).grow(0.15F, 0.05F, 0.15F).grow((float)dimensions.getX(), 0.0F, (float)dimensions.getZ()).add(0.0F, 0.0F, 0.0F, 0.0F, (float)dimensions.getY(), 0.0F);
    }
}
