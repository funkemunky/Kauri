    ...
    public void onPacket(Object packet, String packetType, long timeStamp) {
        ...
        if(packetType.equals(Packet.Client.ARM_ANIMATION)) {
            ...
            if(target != null && getData().getLastLogin().hasPassed(5) && !getData().isServerPos() && getData().getTransPing() < 450) {
                long range = (move.getYawDelta() > 4 ? 250 : 200) + Math.abs(getData().getTransPing() - getData().getLastTransPing()) * 3;
                ...
                if(distance > 3.08F && getData().getMovementProcessor().getLagTicks() == 0) {
                    if(vl++ > 12) {
