...
public class BlockC extends Check {
    ...
    @Packet
    public void onBlockPlace(WrapperPlayClientPlayerBlockPlacement event) {
        Location loc = new Location(data.getPlayer().getWorld(), event.getBlockPosition().getX(),
                event.getBlockPosition().getY(), event.getBlockPosition().getZ());
